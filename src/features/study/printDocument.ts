...
/**
 * That a model wrote part of this page, and that we cannot vouch for the rest.
 *
 * # Why both halves are one sentence
 *
 * The first half is the disclosure. The second is what makes it honest.
 *
 * A notice reading "parts of this document were generated by AI" quietly
 * certifies that the remainder was not — and that is a claim this application
 * cannot support. It knows a model wrote an answer because it *recorded the
 * model id at the time*. About text the student typed into a note it knows
 * nothing, and cannot: a paragraph pasted out of a chatbot and a paragraph
 * copied from a textbook arrive through the same keystroke.
 *
 * For a supervisor who relies on the footer, a certification we cannot back is
 * worse than no footer at all. So the notice states what was recorded and then
 * says plainly where its knowledge stops.
 *
 * # Why the application does not try to find out
 *
 * Two mechanisms suggest themselves and both are refused.
 *
 * Detecting machine-written prose by classifier is unreliable in general and
 * systematically wrong on writers working outside their first language — this
 * atlas is used in Bulgarian, Spanish and English by people writing in all
 * three. A tool that accuses a student of cheating on a coin toss has no
 * business in a study aid.
 *
 * Recording that a paragraph was pasted is trivial to build and answers a
 * different question than the one asked. Pasting is not evidence of a model; it
 * is evidence of a textbook, a lecture slide, or the student's own file from
 * last year. Printing it would turn a notebook into surveillance and would
 * produce false accusations at a steady rate.
 */
const AI_NOTICE: Record<Exclude<Language, "auto">, string> = {
  en:
    "Answers and assessments here were generated by an AI assistant and may " +
    "contain errors; the model is named beside each. Notes are entered by the " +
    "student — Anatria3D does not record where their text came from.",
  es:
    "Las respuestas y valoraciones de este documento fueron generadas por un " +
    "asistente de IA y pueden contener errores; el modelo se indica junto a " +
    "cada una. Las notas las introduce el estudiante: Anatria3D no registra la " +
    "procedencia de su texto.",
  bg:
    "Отговорите и оценките в този документ са генерирани от AI асистент и може " +
    "да съдържат грешки; моделът е посочен до всеки от тях. Бележките се " +
    "въвеждат от студента — Anatria3D не записва произхода на техния текст.",
};

/**
 * Did a model write any part of this document?
 *
 * Answered from what was recorded, never inferred from the prose. Three things
 * qualify, and the last two are easy to forget:
 *
 * - an assistant answer carrying the model that wrote it;
 * - the assistant's **verdict** on a case drill, which is its assessment of the
 *   reader and is model-written even though no `model` field sits beside it;
 * - the same, per visit, on a patient record printed as a history.
 *
 * Deliberately excluded: `findings`, `recordUpdates` and `sealedAnswer`. Those
 * look like clinical prose and are not — they are typed into the case form by
 * whoever set the drill up (see `CaseBar.tsx`). Marking them would be as wrong
 * as failing to mark an answer.
 */
export function hasGeneratedText(document: PrintDocument): boolean {
  const anyAnswered = (exchanges: PrintExchange[]) =>
    exchanges.some((exchange) => exchange.role === "assistant" && exchange.model !== null);

  return (
    anyAnswered(document.exchanges) ||
    document.verdict !== null ||
    document.visits.some((visit) => visit.verdict !== null || anyAnswered(visit.exchanges))
  );
}

/**
 * The AI notice, in the reader's language and in English — or nothing.
 *
 * Nothing is the common case for a notebook of the student's own notes, and it
 * is the right answer there: with no disclosure on the page there is no false
 * inference to correct, and printing "we do not record where your text came
 * from" beside somebody's own handwriting reads as an insinuation rather than a
 * notice. The two halves travel together or not at all.
 *
 * Language follows `disclaimers` exactly, for the same reason: the page travels
 * further than the student, and English is what a supervisor most likely reads.
 */
export function aiNotice(document: PrintDocument): string[] {
  if (!hasGeneratedText(document)) return [];
  const language = document.language;
  if (language === "auto") return [AI_NOTICE.bg, AI_NOTICE.es, AI_NOTICE.en];
  return language === "en" ? [AI_NOTICE.en] : [AI_NOTICE[language], AI_NOTICE.en];
}
